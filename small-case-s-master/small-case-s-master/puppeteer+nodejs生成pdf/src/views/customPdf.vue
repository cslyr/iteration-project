<template>
  <a-button style="position: absolute;right: 200px;top: 2px;z-index: 999;" type="primary" @click="save">保存</a-button>
  <a-button style="position: absolute;right: 80px;top: 2px;z-index: 999;" type="primary"
    @click="downloadPdf">保存为pdf</a-button>
  <div class="main">
    ...
    <div class="content">
      ...
      <!-- 展示 -->
      <div class="preview" :border="false" id="preview" style="height: 90%;">
        <div class="divider" v-for="item in state.pageNum" :style="{ top: (1090 * item) + 'px' }">
          -----------------------------------------------------------
          第{{ item }}页到此结束------------------------------------------------
        </div>
        <div v-for="(item, index) in moduleList.filter(row => row.switch) " :key="item.id">
...
import { RightOutlined, DownOutlined, DeleteOutlined, EditOutlined, ArrowUpOutlined, ArrowDownOutlined, LoadingOutlined, PlusOutlined } from '@ant-design/icons-vue'
...
const moduleList = ref([
  {
    ...
    // 更多信息
    moreInfo: [
      { key: 'wxCode', label: '微信号', value: '', type: 'input', icon: '/public/person.png', showIcon: true },
      ...
      [
        {
          moduleItemName: '',
          formItemList: [
            { key: 'avatar', label: '头像', value: '/public/person.png', type: 'avatar', icon: '', showIcon: true },
...
const save = async () => {
  let data = {
    resume_id: '1',
    resume_content: JSON.stringify(moduleList.value)
  }
  let res = await axios({ url: 'http://127.0.0.1:3000/saveResume', method: "post", data: qs.stringify(data) })
  if (res.data.code != 200) return message.error(res.message)
  message.success(res.data.message)
}
// 下载pdf
const downloadPdf = async () => {
  let res = await axios({ url: 'http://127.0.0.1:3000/downloadPdf', method: 'post', data: qs.stringify({ resume_id: 1 }), responseType: 'blob' })
  const blob = new Blob([res.data]);
  const aLabel = document.createElement('a');
  aLabel.download = 'testpdf.pdf';
