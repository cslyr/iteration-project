...
function joinRoom(type?: string, item?: any) {
  ...
  socket.emit('joinRoom', { userId, roomId: type ? item.roomId : roomId.value, userName: userName.value, avatar: avatar.value })
...
const video = () => {
  ...
  nextTick(() => {
    ...
    getLocalStream(videoCallState.localVideo, async () => {
      ...
      createOffer(() => {
        console.log('page1 emit offerSdp' + moment(new Date()).format('h:mm:ss a'), commonState.peer);
...
// 建立socket连接并监听消息
const connection = () => {
  socket = io('http://192.168.0.111:8888')
  // 监听建立连接
  ...
  socket.on('video', (data) => {
    ...
    console.log('page2 receive offerSdp', offerSdp);
    fromId.value = data.fromId
    fromSocketId.value = data.fromSocketId
...
const createOffer = async (icecandidateCallback, el?) => {
  // 接收到远程的媒体流
  commonState.peer.ontrack = (e) => {
    if (el) {
      el.srcObject = e.streams[0]
      console.log('page1 ontrack' + moment(new Date()).format('h:mm:ss a'));
    }
  }
  // 监听 RTCPeerConnection 的 onicecandidate 事件，当 ICE 服务器返回一个新的候选地址时，就会触发该事件
  commonState.peer.onicecandidate = async (event: any) => {
    if (event.candidate) {
      console.log('page1 onicecandidate ' + moment(new Date()).format('h:mm:ss a'), event.candidate);
  ...
  console.log('page1 setLocalDescription ' + moment(new Date()).format('h:mm:ss a'));
}
// 创建answerSdp
const createAnswer = async (icecandidateCallback, el?) => {
  ...
  commonState.peer.ontrack = (e) => {
    if (el) {
      el.srcObject = e.streams[0]
      console.log('page2 ontrack' + moment(new Date()).format('h:mm:ss a'));
    }
  }
  commonState.peer.onicecandidate = async (event: any) => {
    if (event.candidate) {
      console.log('page2 onicecandidate ' + moment(new Date()).format('h:mm:ss a'), event.candidate);
      commonState.answerSdp = JSON.stringify(commonState.peer.localDescription)
      if (commonState.answerSdp) icecandidateCallback()
    }
  }
  console.log('p2 setRemoteDescription before');
  await commonState.peer.setRemoteDescription(JSON.parse(commonState.offerSdp))
  console.log('p2 setRemoteDescription after');

  const answer = await commonState.peer.createAnswer()
  await commonState.peer.setLocalDescription(answer)
  console.log('page2 setLocalDescription ' + moment(new Date()).format('h:mm:ss a'));
}
